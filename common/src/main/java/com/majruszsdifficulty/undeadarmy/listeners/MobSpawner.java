...
public class MobSpawner {
    ...
    private static void tryToSpawn(OnUndeadArmyTicked data) {
        if (!TimeHelper.haveSecondsPassed(1.0) || data.undeadArmy.phase.state != UndeadArmy.Phase.State.WAVE_ONGOING) {
            return;
        }
    ...
    private static void generateMobList(OnUndeadArmyStateChanged data) {
        ...
        float sizeMultiplier = 1.0f + (data.undeadArmy.participants.size() - 1) * (float) UndeadArmyConfig.get().extraSizeRatioPerPlayer();
        UndeadArmyConfig.WaveDef waveDef = UndeadArmyConfig.get().waves().get(data.undeadArmy.currentWave);
    ...
    private static Vec3 buildOffset(UndeadArmy undeadArmy) {
        int spawnRadius = UndeadArmyConfig.get().areaRadius() - 15;
